...
from deepcore.nets.STGCN import layers
...
class STGCNGraphConv(nn.Module):
    ...
    def __init__(self, temporal_kernel_size, spatio_kernel_size, act_func, graph_conv_type, gso, enable_bias, droprate,  blocks, n_vertex, n_his):
        super(STGCNGraphConv, self).__init__()
        modules = []
        for l in range(len(blocks) - 3):
            modules.append(layers.STConvBlock(temporal_kernel_size, spatio_kernel_size, n_vertex, blocks[l][-1], blocks[l+1], act_func, graph_conv_type, gso, enable_bias, droprate))
        self.st_blocks = nn.Sequential(*modules)
        Ko = n_his - (len(blocks) - 3) * 2 * (temporal_kernel_size - 1)
        self.Ko = Ko
        if self.Ko > 1:
